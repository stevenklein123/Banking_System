...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($account_id) || empty($amount) || empty($payment_method)) {
        ...
        $result = processWithdrawal(intval($account_id), floatval($amount), $payment_method, $_SESSION['user_id'], $description);
        if ($result['success']) {
            $success = 'Withdrawal processed successfully! Reference: ' . $result['reference_number'] . ' New Balance: ' . formatCurrency($result['new_balance']);
            $account_id = '';
            $account_number = '';
        } else {
...
<head>
    ...
    <title>Process Withdrawal - Banking System</title>
            ...
            <aside class="sidebar">
                <nav class="sidebar-nav">
                    <ul>
                        ...
                        <li><a href="withdrawal.php" class="active">Process Withdrawal</a></li>
